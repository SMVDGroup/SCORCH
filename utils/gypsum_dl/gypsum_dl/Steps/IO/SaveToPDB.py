...
def convert_sdfs_to_PDBs(contnrs, output_folder):
    ...
    for contnr in contnrs:
        ...
        for i, m in enumerate(mols):
            ...
            ########################################################################
            # edited for SCORCH so that naming convention matches normal processing
            # pipeline
            ########################################################################
            pdb_file = "{}{}.pdb".format(
            ...
            if mol == None:
                ...
                ########################################################################
                # edited for SCORCH to remove hydrogens before saving to match the
                # normal preprocessing pipeline (hydrogens added by MGLTools 1.5.6)
                ########################################################################
