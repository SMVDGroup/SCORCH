...
def prepare_molecules(args):
    ...
    ########################################################################
    # edited for SCORCH to allow passing smile and name as tuple inside list
    # instead of supplying a file to the program

    # original code is commented out and replaced with:
...
def merge_parameters(default, params):
    ...
    ########################################################################
    # added for SCORCH to allow passing smile and name as tuple inside list
    # instead of supplying a filename to the program
    ########################################################################
    type_dict['source'] = type(list())
    ########################################################################
...
def make_type_dict(dictionary):
    ...
    ########################################################################
    # added for SCORCH to allow passing smile and name as tuple inside list
    # instead of supplying a filename to the program as str
    ########################################################################
    allowed_types = [int, float, bool, str, list]
    ########################################################################
...
def finalize_params(params):
    ...
    ########################################################################
    # commented out for SCORCH so that program does not exit with source
    # argument supplied as SMILES string and not a filename
    ########################################################################
