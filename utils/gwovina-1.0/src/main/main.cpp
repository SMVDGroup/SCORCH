/*
        GWOVina version 1.0                     Date: 27/5/2020

        This file is revised from main.cpp in AutoDock Vina.
        Authors: Michael K. M. Wong  <emailtoming15@hotmail>
...
#include "parallel_gwo.h" //gwovina
#include "file.h"
#include "cache.h"
#include "non_cache.h"
...
void do_search(model& m, const boost::optional<model>& ref, const scoring_function& sf, const precalculate& prec, const igrid& ig, const precalculate& prec_widened, const igrid& ig_widened, non_cache& nc, // nc.slope is changed
			   ...
			   const parallel_gwo& par/*GWOVina*/, fl energy_range, sz num_modes, 
			   int seed, int verbosity, bool score_only, bool local_only, tee& log, const terms& t, const flv& weights, int num_wolves) {
	conf_size s = m.get_size();
	//m.write_structure("aaa.txt");
	conf c = m.get_initial_conf();

	//for (int i = 0; i < c2.ligands[0].torsions.size();i++)
	//{
		//log << c2.ligands[0].torsions[i] << '\n';
	//}
	fl e = max_fl; 
	//lf e2 = max_fl;
	...
	if(score_only) {
		...
		//log << "	intramolecular_energy		:" << intramolecular_energy << '\n' ;
		//fl re = m.eval_without_ligand(prec, authentic_v, c);
		//log << "	receptor energy		:" << re << '\n' ;

		naive_non_cache nnc(&prec); // for out of grid issues

		//fl e3 = m.eval(prec, nnc, authentic_v, c);
		//log << "	energy		:" << e3 << '\n';
		e = m.eval_adjusted(sf, prec, nnc, authentic_v, c, intramolecular_energy);
		//e2 = m.eval_adjusted(sf, prec, nnc, authentic_v, c, re);

		log << "Affinity: " << std::fixed << std::setprecision(5) << e << " (kcal/mol)";
		log.endl();

		//log << "Affinity2: " << std::fixed << std::setprecision(5) << e2 << " (kcal/mol)";

		//log.endl();
		flv term_values = t.evale_robust(m);
		//flv term_values = t.evale(m);
		//t.display_info();
		//factors termsss = t.eval(m);
		
		VINA_CHECK(term_values.size() == 5);

		log << "Intermolecular contributions to the terms, before weighting:\n";
		log << std::setprecision(5);
		log << "    gauss 1     : " << term_values[0] /*<< "	" << termsss.e[0] <<	"	" << termsss.i[0]*/ <<'\n';
		log << "    gauss 2     : " << term_values[1]  /*<< "	" << termsss.e[1] <<	"	" << termsss.i[1]*/ <<'\n';
		log << "    repulsion   : " << term_values[2]  /*<< "	" << termsss.e[2] <<	"	" << termsss.i[2]*/ <<'\n';
		log << "    hydrophobic : " << term_values[3]  /*<< "	" << termsss.e[3] <<	"	" << termsss.i[3]*/ <<'\n';
		log << "    Hydrogen    : " << term_values[4]  /*<< "	" << termsss.e[4] <<	"	" << termsss.i[4]*/ <<'\n';
		...
		refine_structure(m, prec, nc, out, authentic_v, par.gwo.ssd_par.evals/*GWOVina*/);
		done(verbosity, log);
		...
		par(m, out_cont, prec, ig, prec_widened, ig_widened, corner1, corner2, generator, num_wolves);
		done(verbosity, log);
...
if(!out_cont.empty()) {
		...
		VINA_FOR_IN(i, out_cont)
			refine_structure(m, prec, nc, out_cont[i], authentic_v, par.gwo.ssd_par.evals/*GWOVina*/);
...
void main_procedure(model& m, const boost::optional<model>& ref, // m is non-const (FIXME?)
				 ...
				 const flv& weights,
				 int cpu, int seed, int verbosity, sz num_modes, fl energy_range, tee& log, const std::string& rotamer_name, int num_wolves) {
	...
	parallel_gwo par;/*GWOVina*/
	sz heuristic = m.num_movable_atoms() + 10 * m.get_size().num_degrees_of_freedom();
	//par.gwo.num_steps = unsigned(70 * 3 * (50 + heuristic) / 2); // 2 * 70 -> 8 * 20 // FIXME/*GWOVina*/
	par.gwo.ssd_par.evals = unsigned((25 + m.num_movable_atoms()) / 3);/*GWOVina*/
	par.gwo.min_rmsd = 1.0;/*GWOVina*/
	par.gwo.num_saved_mins = 50;/*GWOVina*/
	par.gwo.hunt_cap = vec(10, 10, 10);/*GWOVina*/
	...
	if(randomize_only) {
		...
		if(no_cache) {
			do_search(m, ref, wt, prec, nc, prec_widened, nc_widened, nc,
					  ...
					  seed, verbosity, score_only, local_only, log, t, weights, num_wolves);
		}
		else {
...
int main(int argc, char* argv[]) {
	...
	const std::string version_string = "GWOVina version 1.0";    //gwo
	const std::string error_message = "\n\n\
Please contact author, Michael, K. M. Wong <emailtoming15@hotmail.com> or Dr. Shirley W. I. Siu <shirleysiu@umac.mo>, so\n\
...
=======================================================================\n";
	...
	try {
		std::string rigid_name, ligand_name, flex_name, config_name, out_name, log_name, rotamer_name;
		...
		int cpu = 0, seed, exhaustiveness, verbosity = 2, num_modes = 9,num_wolves = 0;
		fl energy_range = 2.0;
		...
		inputs.add_options()
			...
			("rotamer", value<std::string>(&rotamer_name), "rotamer file")
		...
		;
		options_description gwo("GWO parameters (optional)");
		gwo.add_options()
			("num_wolves", value<int>(&num_wolves)->default_value(12), "Number of wolves per thread")
		;
		...
		desc       .add(inputs).add(search_area).add(gwo).add(outputs).add(advanced).add(misc).add(config).add(info);
		desc_config.add(inputs).add(search_area).add(gwo).add(outputs).add(advanced).add(misc);
		desc_simple.add(inputs).add(search_area).add(gwo).add(outputs).add(misc).add(config).add(info);
		...
		boost::optional<std::string> rotamer_name_opt;
		if(vm.count("rotamer"))
			rotamer_name_opt = rotamer_name;
		...
		//m.write_structure("12345.dat");
		rotamer rotamers(rotamer_name);
		m.rotamers = rotamers;
		//rotamers.print();
		...
		main_procedure(m, ref, 
					...
					gd, exhaustiveness,
					weights,
					cpu, seed, verbosity, max_modes_sz, energy_range, log, rotamer_name, num_wolves);
	}
	catch(file_error& e) {
