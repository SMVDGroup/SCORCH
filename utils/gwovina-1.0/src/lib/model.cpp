	...
	VINA_FOR_IN(i, atoms)
		VINA_FOR_IN(j, m.atoms) {
			...
			if(t1 < n && t2 < n) {
				...
				{
					my_flex_pairs.push_back(interacting_pair(type_pair_index, new_i, new_j));
				}
			}
		}
...
fl model::eval_without_ligand(const precalculate& p, const vec& v, const conf& c) {
	set(c);
	fl e = 0;
	/*
	// internal for each ligand
